...
import {navigates} from './navigation/RootNavigation';
...
const DashBoard = () => {
  ...
  const addTask = async (values: any) => {
    const param = {
      ...
      projectId: values?.p_name,
      taskCategoryId: values?.category,
      responsiblePersonId: values?.assignedTo,
      taskCreatorId: user?.userInfo?.AgentId,
      taskOwnerId: 27,
      taskObserverId: '',
      taskParticipantsId: '',
      startDatetime: moment().format('YYYY-MM-DD') + ' 00:00:00',
      finishDatetime:
        moment(values?.dueDate).format('YYYY-MM-DD') + ' 00:00:00',
      duaration: '10',
      statusFlag: 0,
      taskStageId: 1,
      ratePerTask: 25,
  ...
  const ToDoItem = ({
    ...
    <View style={styles.todoContainer}>
      <View style={styles.todoInfoContainer}>
        <Checkbox checked={todo.isDone} onPress={() => handleDone(todo.id)} />
        <Text
          style={[
            styles.todoText,
        ...
        <>
          <View style={{}}>
            <View style={styles.header}>
              <TouchableOpacity
                onPress={() => Alert('Clicked!')}></TouchableOpacity>
              <TouchableOpacity
            ...
            <View
              ...
              <TouchableOpacity
                onPress={getTodos}
                style={[styles.dateButton, {backgroundColor: '#0078D4'}]}>
            ...
            {showSuggestions && mentionQuery.length > 0 && (
              <View style={styles.suggestionsContainer}>
                <FlatList
                  ...
                  renderItem={({item}) => (
                    <TouchableOpacity
                      onPress={() => handleSelectAgent(item.AgentName)}>
