...
export default function AttendanceHistory() {
  ...
  const getAttendanceList = async (
    month: any = selectedMonth,
    year: any = selectedYear,
  ) => {
    try {
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.headerRow}>
        ...
        <Text style={styles.header}>
          Attendance Of {user?.userInfo?.AgentName}
        </Text>
      </View>
      <View
        ...
        <DropdownModal
          ...
          value={selectedMonth}
          onSelect={(item: any) =>
            getAttendanceList({value: item.value}, selectedYear)
          }
        />

        <Text style={{fontSize: 18, marginLeft: 16, marginRight: 8}}>
          Year:
        </Text>
        <DropdownModal
          data={yearOptions}
          value={selectedYear}
          onSelect={(item: any) =>
            getAttendanceList(selectedMonth, {value: item.value})
          }
        />
      </View>
